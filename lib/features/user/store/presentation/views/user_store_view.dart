...
class UserStoreView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => sl<UserStoreCubit>()..fetchPackages(),
      child: const _UserStoreContent(),
    );
  }
}

class _UserStoreContent extends StatelessWidget {
  const _UserStoreContent();
      ...
      body: BlocListener<UserStoreCubit, DriverStoreState>(
        ...
        child: Column(
          children: [
            _buildPointsHeader(),
            Expanded(
              child: BlocBuilder<UserStoreCubit, DriverStoreState>(
                  ...
                  if (package.priceMoney != null && package.priceMoney! > 0)
                    Expanded(
                      child: ElevatedButton.icon(
                        ...
                        // Using 'cash' per UserPackageController
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.pop(ctx);
              context.read<UserStoreCubit>().buyPackage(package.id, method);
            },
            style: ElevatedButton.styleFrom(
                backgroundColor: AppColors.primaryColor),
